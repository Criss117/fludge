...
export function GroupMembersSection({ groupId }: { groupId: string }) {
  ...
  const [memberToRemove, setMemberToRemove] = useState<MemberSummary | null>(null);
  ...
  return (
    <View className="flex-1 gap-y-2">
      <SearchInput query={query} setQuery={setQuery} placeholder="Buscar Miembros" />
      <FlatList
        data={members}
        className="flex-1"
        ...
        ListEmptyComponent={
          <View className="flex-1 items-center justify-center py-12">
            ...
            <Typography.Paragraph color="muted">No hay miembros</Typography.Paragraph>
          </View>
        }
      />
      <Dialog
