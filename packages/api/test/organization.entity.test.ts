...
import { Organization } from "../src/modules/iam/organization/domain/entities/organization.entity";
...
const createOrganization = () => {
  ...
  return {
    organization: Organization.create({
      ...
      address: "Main Street",
      phone: "555-0100",
      groups: [{ name: "Editors", description: null, permissions: Permissions.create({ groups: ["read"], products: ["read"] }), createdBy: null }],
...
describe("Organization", () => {
  ...
  it("adds, queries, and removes group members", () => {
    ...
    expect(organization.getMembersOfGroup(UUID.fromString(group.id))).toHaveLength(1);
    expect(organization.values.groupMembers).toHaveLength(1);
    expect(() => organization.addGroupMember(UUID.fromString(group.id), owner.id, createdBy)).toThrow(GroupMemberAlreadyExistsException);
  ...
  it("rejects unknown groups and members when assigning", () => {
    ...
    const owner = organization.members.owner!;
    expect(() => organization.addGroupMember(UUID.generate(), owner.id, owner.id)).toThrow(GroupNotFoundException);
    const group = organization.groups.values(organization.id)[0]!;
    expect(() => organization.addGroupMember(UUID.fromString(group.id), UUID.generate(), owner.id)).toThrow(MemberNotFoundException);
  ...
  it("grants owners all permissions and members only group permissions", () => {
    const { organization } = createOrganization();
    const owner = organization.members.owner!;
    expect(organization.memberHasPermission(owner.id, { sales: ["delete"] })).toBe(true);
    const member = Member.create({ userId: UUID.generate(), assignedBy: owner.id, role: "member" });
    organization.members.addMember(member);
    const group = organization.groups.values(organization.id)[0]!;
    organization.addGroupMember(UUID.fromString(group.id), member.id, owner.id);
    expect(organization.memberHasPermission(member.id, { groups: ["read"] })).toBe(true);
    expect(organization.memberHasPermission(member.id, { groups: ["delete"] })).toBe(false);
    expect(organization.memberHasPermission(UUID.generate(), { groups: ["read"] })).toBe(false);
  });
});
