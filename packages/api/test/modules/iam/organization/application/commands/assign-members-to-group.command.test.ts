...
function makeActiveOrganization() {
  ...
  const organization = Organization.create({
    name: "Acme", legalName: "Acme", taxId: "TAX", address: "Street", phone: "555",
    owner: { userId: loggedUserId, role: "owner", assignedBy: null },
  });
  const group = Group.create({ name: "Editors", description: "Editors", permissions: Permissions.create({ groups: ["read"] }), createdBy: organization.members.owner!.id });
...
function addMember(organization: Organization, userId: string) {
  const member = Member.create({ userId: UUID.fromString(userId), role: "member", assignedBy: null });
...
describe("AssignMembersToGroupCommand", () => {
  it("assigns multiple members and saves only groupMembers", async () => {
    ...
    const { activeOrganization, loggedUserId, group } = makeActiveOrganization();
    const memberOne = addMember(activeOrganization, "member-1");
    const memberTwo = addMember(activeOrganization, "member-2");

    const result = await command.execute(loggedUserId.toString(), activeOrganization, { groupId: group.id.toString(), memberIds: [memberOne.id.toString(), memberTwo.id.toString()] });
  ...
  it("throws NOT_FOUND for an unknown group", async () => {
    const repository = makeRepository(); const command = new AssignMembersToGroupCommand(repository as any);
    const { activeOrganization, loggedUserId } = makeActiveOrganization();
    await expect(command.execute(loggedUserId.toString(), activeOrganization, { groupId: "missing", memberIds: ["member-1"] })).rejects.toBeInstanceOf(GroupNotFoundException);
    expect(repository.save).not.toHaveBeenCalled();
  });
  it("throws NOT_FOUND for an unknown member", async () => {
    const repository = makeRepository(); const command = new AssignMembersToGroupCommand(repository as any);
    const { activeOrganization, loggedUserId, group } = makeActiveOrganization();
    await expect(command.execute(loggedUserId.toString(), activeOrganization, { groupId: group.id.toString(), memberIds: ["missing"] })).rejects.toBeInstanceOf(MemberNotFoundException);
    expect(repository.save).not.toHaveBeenCalled();
  });
  it("throws CONFLICT for a duplicate group-member pair", async () => {
    const repository = makeRepository(); const command = new AssignMembersToGroupCommand(repository as any);
    const { activeOrganization, loggedUserId, group } = makeActiveOrganization(); const member = addMember(activeOrganization, "member-3");
    await command.execute(loggedUserId.toString(), activeOrganization, { groupId: group.id.toString(), memberIds: [member.id.toString()] });
    await expect(command.execute(loggedUserId.toString(), activeOrganization, { groupId: group.id.toString(), memberIds: [member.id.toString()] })).rejects.toBeInstanceOf(GroupMemberAlreadyExistsException);
  });
  it("throws INTERNAL_SERVER_ERROR when saving fails", async () => {
    const repository = makeRepository(err(new Error("boom"))); const command = new AssignMembersToGroupCommand(repository as any);
    const { activeOrganization, loggedUserId, group } = makeActiveOrganization(); const member = addMember(activeOrganization, "member-4");
    await expect(command.execute(loggedUserId.toString(), activeOrganization, { groupId: group.id.toString(), memberIds: [member.id.toString()] })).rejects.toMatchObject({ code: "INTERNAL_SERVER_ERROR" });
  });
});
