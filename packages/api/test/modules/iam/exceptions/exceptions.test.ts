...
import { OwnerNotFoundException } from "@fludge/api/modules/iam/organization/domain/exceptions/owner-not-found.exception";
...
describe("domain exceptions", () => {
  ...
  it("allows custom messages", () => {
    expect(new GroupNotFoundException("Missing group").message).toBe(
      "Missing group",
    );
    expect(new OwnerNotFoundException("Missing owner").message).toBe(
      "Missing owner",
    );
  });
});
