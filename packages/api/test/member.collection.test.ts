...
import { MemberCollection } from "../src/modules/iam/organization/domain/entities/member.collection";
...
const createMember = (role: "member" | "owner" = "member") => Member.create({ userId: UUID.generate(), assignedBy: null, role });
...
describe("MemberCollection", () => {
  it("creates empty or initialized collections and adds members", () => {
    ...
    const collection = MemberCollection.create([{ userId: UUID.generate(), assignedBy: null, role: "member" }]);
  ...
  it("enforces unique IDs and a single owner", () => {
    ...
    const owner = createMember("owner");
    collection.addMember(owner);
    expect(() => collection.addMember(owner)).toThrow(MemberAlreadyExistsException);
  ...
  it("finds by ID or user ID and removes regular members only", () => {
    ...
    expect(() => collection.removeMember(owner.id)).toThrow(CantRemoveOwnerException);
    expect(() => collection.removeMember(UUID.generate())).toThrow(MemberNotFoundException);
  });
});
